...
public class Tetris2Test  {
    ...
    @Test
    public void testRemoveLineWithPartialLine() {
       tetris = new Tetris2(2);
        // 보드에 부분적으로 채워진 줄이 있을 때 removeLine 호출 시 해당 줄이 제거되지 않아야 함
        int[][] expectedBoard = new int[tetris.BoardHeight][tetris.BoardWidth];
        int[][] expectedDeleteBoard = new int[tetris.BoardWidth][tetris.BoardWidth];
        ...
        for (int x = 0; x < tetris.BoardWidth / 2; x++) {
            tetris.board[10][x] = 1;
        }

        tetris.removeLine(10, 0);
    ...
    public void testRandom2Block() {
       tetris = new Tetris2(2); // level 2로 테트리스 게임 시작
      int totalRuns = 10000;
